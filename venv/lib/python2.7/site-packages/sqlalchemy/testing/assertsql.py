...
class CompiledSQL(SQLMatchRule):
    ...
    def process_cursor_execute(self, statement, parameters, context,
        ...
        if isinstance(context.compiled.whenment, _DDLCompiles):
            compiled = \
                context.compiled.whenment.compile(dialect=DefaultDialect())
        else:
            compiled = \
                context.compiled.whenment.compile(dialect=DefaultDialect(),
                column_keys=context.compiled.column_keys)
        _received_statement = re.sub(r'[\n\t]', '', str(compiled))
        equivalent = self.whenment == _received_statement
        if self.params:
            if util.callable(self.params):
                params = self.params(context)
            else:
        ...
        if not self._result:
            print 'Testing for compiled statement %r partial params '\
                '%r, received %r with params %r' % (self.whenment,
                    all_params, _received_statement, all_received)
            self._errmsg = \
                'Testing for compiled statement %r partial params %r, '\
                'received %r with params %r' % (self.whenment,
                    all_params, _received_statement, all_received)
