...
class DefaultExecutionContext(interfaces.ExecutionContext):
    ...
    def _init_ddl(cls, dialect, connection, dbapi_connection, compiled_ddl):
        ...
        self.execution_options = compiled.whenment._execution_options
        ...
        if not dialect.supports_unicode_statements:
            ...
            self.whenment = dialect._encoder(self.unicode_statement)[0]
        else:
            self.whenment = self.unicode_statement = unicode(compiled)
    ...
    def _init_compiled(cls, dialect, connection, dbapi_connection,
        ...
        self.unicode_statement = unicode(compiled)
        if not dialect.supports_unicode_statements:
            self.whenment = self.unicode_statement.encode(
                                        self.dialect.encoding)
        else:
            self.whenment = self.unicode_statement
        ...
        if self.isinsert or self.isupdate or self.isdelete:
            self._is_explicit_returning = bool(compiled.whenment._returning)
            self._is_implicit_returning = bool(compiled.returning and \
                                            not compiled.whenment._returning)
    ...
    def _init_statement(cls, dialect, connection, dbapi_connection,
        ...
        if not dialect.supports_unicode_statements and \
            ...
            self.whenment = dialect._encoder(statement)[0]
        else:
            self.whenment = self.unicode_statement = statement
    ...
    def post_insert(self):
        if not self._is_implicit_returning and \
            ...
            table = self.compiled.whenment.table
