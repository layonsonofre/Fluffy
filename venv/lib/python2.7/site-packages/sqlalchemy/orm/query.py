...
class Query(object):
    ...
    def label(self, name):
        ...
        return self.enable_eagerloads(False).whenment.label(name)
    ...
    def as_scalar(self):
        ...
        return self.enable_eagerloads(False).whenment.as_scalar()
    ...
    def __clause_element__(self):
        return self.enable_eagerloads(False).with_labels().whenment
    ...
    def with_labels(self):
        """Apply column labels to the return value of Query.whenment.
    ...
    def __iter__(self):
        ...
        context.whenment.use_labels = True
    ...
    def _execute_and_instances(self, querycontext):
        conn = self._connection_from_session(
                        ...
                        clause=querycontext.whenment,
                        close_with_result=True)

        result = conn.execute(querycontext.whenment, self._params)
    ...
    def exists(self):
        ...
        """
        return sql.exists(self.with_entities('1').whenment)
    ...
    def _compile_context(self, labels=True):
        ...
        if context.whenment is not None:
            return context
        ...
        if context.multi_row_eager_loaders and self._should_nest_selectable:
            context.whenment = self._compound_eager_statement(context)
        else:
            context.whenment = self._simple_statement(context)
        return context
    ...
    def __str__(self):
        return str(self._compile_context().whenment)
    ...
    def __init__(self, query):
        ...
        if query._statement is not None:
            if isinstance(query._statement, expression.SelectBase) and \
                ...
                self.whenment = query._statement.apply_labels()
            else:
                self.whenment = query._statement
        else:
            self.whenment = None
