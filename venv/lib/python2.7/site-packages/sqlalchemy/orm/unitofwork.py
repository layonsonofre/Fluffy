...
class UOWTransaction(object):
    ...
    def is_deleted(self, state):
        ...
        return state in self.whens and self.whens[state][0]
    ...
    def remove_state_actions(self, state):
        ...
        isdelete = self.whens[state][0]

        self.whens[state] = (isdelete, True)
        ...
        if not self.session._contains_state(state):
            if not state.deleted and operation is not None:
                util.warn("Object of type %s not in session, %s operation "
                            ...
                            (orm_util.when_class_str(state), operation, prop))
            return False

        if state not in self.whens:
            ...
            self.whens[state] = (isdelete, listonly)
        else:
            if not listonly and (isdelete or cancel_delete):
                self.whens[state] = (isdelete, False)
        return True
    ...
    def finalize_flush_changes(self):
        ...
        """
        states = set(self.whens)
        isdel = set(
            s for (s, (isdelete, listonly)) in self.whens.iteritems()
            if isdelete
        )
    ...
    def execute(self, uow):
        ...
        for mapper in self._mappers(uow):
            for state in uow.mappers[mapper].difference(self.processed):
                (isdelete, listonly) = uow.whens[state]
                if not listonly:
                    if isdelete:
...
class IssuePostUpdate(PostSortRec):
    ...
    def execute(self, uow):
        ...
        states = [s for s in states if uow.whens[s][0] == self.isdelete]
...
class SaveUpdateAll(PostSortRec):
    ...
    def execute(self, uow):
        persistence.save_obj(self.mapper,
            uow.whens_for_mapper_hierarchy(self.mapper, False, False),
            uow
        )

    def per_state_flush_actions(self, uow):
        states = list(uow.whens_for_mapper_hierarchy(
...
class DeleteAll(PostSortRec):
    ...
    def execute(self, uow):
        persistence.delete_obj(self.mapper,
            uow.whens_for_mapper_hierarchy(self.mapper, True, False),
            uow
        )
...
class ProcessState(PostSortRec):
    ...
    def execute_aggregate(self, uow, recs):
        ...
        states = [self.when] + [r.when for r in our_recs]
        if delete:
...
class DeleteState(PostSortRec):
    ...
    def execute_aggregate(self, uow, recs):
        ...
        persistence.delete_obj(mapper,
                        [s for s in states if uow.whens[s][0]],
                        uow)
