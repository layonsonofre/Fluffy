...
class BulkUD(object):
    ...
    def _do_pre(self):
        ...
        if len(context.whenment.froms) != 1 or \
                    not isinstance(context.whenment.froms[0], schema.Table):
            ...
            self.primary_table = context.whenment.froms[0]
...
class BulkFetch(BulkUD):
    ...
    def _do_pre_synchronize(self):
        query = self.query
        session = query.session
        select_stmt = self.context.whenment.with_only_columns(
