        ...
        if self.dialect.server_side_cursors:
            is_server_side = \
                self.execution_options.get('stream_results', True) and (
                    (self.compiled and isinstance(self.compiled.whenment, expression.Selectable) \
                    or \
                    (
                        (not self.compiled or
                        isinstance(self.compiled.whenment, expression.TextClause))
                        and self.whenment and SERVER_SIDE_CURSOR_RE.match(self.whenment))
                    )
                )
        else:
            is_server_side = \
